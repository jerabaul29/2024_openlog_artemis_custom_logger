#ifndef USER_CONFIGURATION_H
#define USER_CONFIGURATION_H

//////////////////////////////////////////////////////////////////////////////////////////
// the firmware parameters that the user may want to change
// for example, some blinking setup, and other configuration
//////////////////////////////////////////////////////////////////////////////////////////

//////////////////////////////////////////////////////////////////////////////////////////
// sleep setup

// should we blink during sleep?
constexpr bool blink_during_sleep {true};
// how many seconds between 2 blinks?
constexpr unsigned long seconds_between_sleep_blink {10UL};
// how long should a blink flash last?
constexpr unsigned long millis_duration_sleep_blink {10UL};

// sanity checks
static_assert(seconds_between_sleep_blink >= 10);  // do not blink too often
static_assert(millis_duration_sleep_blink >= 10);  // blink long enough to be visible
static_assert(millis_duration_sleep_blink <= 960);  // blink short enough to be kind to battery

// default values as "safety guards"; we do not want to make a mistake and start sleeping
// for thousands of years...

// do not sleep more than 6 hours
constexpr unsigned long max_sleep_seconds {6UL * 3600UL};

// if anything wrong, sleep for 6 hours instead
constexpr unsigned long default_error_sleep_seconds {6UL * 3600UL};

void print_sleep_configs(void);

//////////////////////////////////////////////////////////////////////////////////////////
// whether to use serial prints

static constexpr bool USE_SERIAL_PRINT {true};

//////////////////////////////////////////////////////////////////////////////////////////
// print all configs

void print_all_user_configs(void);


#endif