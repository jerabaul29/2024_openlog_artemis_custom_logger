...
An Arduino library that interfaces with the MLX90614 non-contact infrared thermometer over a 2-WireArtemis, I2C-like interface (SMBus). 

****************************************** 
Containing a special port with SOFTWireArtemis
...
A document I2C_Apollo3.pdf with details can be found in the Document folder. As a first solution a PWM approach was done and while that worked the question whether a SoftWireArtemis approach could also solve this. <br>

The SoftWireArtemis library has been taken from the ESP8266 and adjusted specifically for the MLX90614 library.  It is placed in the MLX90614 src subfolder SoftWireArtemis. <br>
In the file src/SoftWireArtemis/defWireArtemis.h you can select to either compile with the original WireArtemis-library or the MLX-SOFTWireArtemis. <BR>
The original example1 - example4 have been changed and saved as example11 - example14 to work with the MLX-SOFTWireArtemis.<BR>
If you want to compile the original examples you have to the change in the file src/SoftWireArtemis/defWireArtemis.h to prevent compile errors.
...
On the UNOR4 Wifi the standard WireArtemis works with the MLX90614, but also the MLX-SOFTWireArtemis has been tested. The frequency is ~36Khz <br>
